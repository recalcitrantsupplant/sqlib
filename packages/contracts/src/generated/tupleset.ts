...
// Reusable field definitions for TupleSet entity
const tupleSetShape = {
  id: iriString,
...
// Full TupleSet schema for validation
export const tupleSetSchema = z.object(tupleSetShape).strict();
// Create schema - id optional, allows backend to mint it or temporary URNs, normalizes isPartOf, excludes read-only fields
export const tupleSetCreateSchema = z.object({
  ...tupleSetShape,
...
export const tupleSetUpdateSchema = z.preprocess(
  (data: unknown) => {
  ...
  z.object(tupleSetShape).partial().omit({ id: true, currentVersionNumber: true, dateCreated: true, dateModified: true }).strict()
)
  .refine((data) => Object.keys(data).length > 0, {
    message: 'Update must include at least one field',
    path: [],
  });
// TupleSet entity type
export type TupleSet = z.infer<typeof tupleSetSchema>;
// TupleSet creation type
export type TupleSetCreate = z.infer<typeof tupleSetCreateSchema>;
// TupleSet update type
export type TupleSetUpdate = z.infer<typeof tupleSetUpdateSchema>;
