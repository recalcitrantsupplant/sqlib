...
// Reusable field definitions for DataBlock entity
...
// Full DataBlock schema for validation
export const dataBlockSchema = z.object(dataBlockShape).strict();
// Create schema - id optional, allows backend to mint it or temporary URNs, normalizes isPartOf, excludes read-only fields
export const dataBlockCreateSchema = z.object({
  ...dataBlockShape,
...
export const dataBlockUpdateSchema = z.preprocess(
  (data: unknown) => {
  ...
  z.object(dataBlockShape).partial().omit({ id: true, currentVersionNumber: true, dateCreated: true, dateModified: true }).strict()
)
  .refine((data) => Object.keys(data).length > 0, {
    message: 'Update must include at least one field',
    path: [],
  });
// DataBlock entity type
export type DataBlock = z.infer<typeof dataBlockSchema>;
// DataBlock creation type
export type DataBlockCreate = z.infer<typeof dataBlockCreateSchema>;
// DataBlock update type
export type DataBlockUpdate = z.infer<typeof dataBlockUpdateSchema>;
