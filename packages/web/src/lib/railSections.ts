 ...
 * page, then the library-scoped sections that drill down from it, then MCP,
 * then a divider, then Backends. Backends is account-level — libraries point
 * at connections rather than containing them — and the divider says so without
 ...
 *
 * `notebooks` and `mcp` are the odd ones out among the rest: they are screens
 * (`/notebook`, `/mcp-server`), not scopes, and they render without the tree
 * at all —
 ...
 *
 * `build` was the first, and `mcp` replaced it at the same slot; `notebooks`
 * is the second. Each renders at its own route with no per-type navigator, so
 * neither has a scoping entry in the maps below — the `Exclude`
 * keeps that a type error rather than a dead entry somebody has to invent a
 * value for.
 *
 * `mcp` is the one section that is not about a library at all: it is about this
 * deployment's MCP endpoint, which is the same whichever library is active. It
 * sits in the rail anyway because that is where a user looks for places, and a
 * section that ignores the active library is a smaller surprise than a page
 * with no way to reach it.
 *
 * Its route is `/mcp-server`, not `/mcp`, because `/mcp` is the API's own path.
 * Nothing here serves the app and the API from one origin, but a reverse proxy
 * in front of both is an ordinary thing to build.
 */
export const SCREEN_SECTIONS = ['notebooks', 'mcp'] as const;
